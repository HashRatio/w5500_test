...
@return	1 for success else 0.
*/ 
uint16 send(SOCKET s, const uint8 * buf, uint16 len, int8 retry)
{
	uint8 status=0;
	uint16 ret=0;
	uint16 freesize=0;
...
#ifdef __DEF_IINCHIP_DBG__
		debug32("socket %d freesize(%d) empty or error\r\n", s, freesize);
#endif
                } while (freesize < ret);
   ...
   if (len > getIINCHIP_TxMAX(s)) ret = getIINCHIP_TxMAX(s); // check size not to exceed MAX size.
	...
	if
		...
		) 
 	{
 	   ...
 	   ret = 0;
#ifdef __DEF_IINCHIP_DBG__
	debug32("%d Fail[%.2x.%.2x.%.2x.%.2x, %.d, %d]\r\n",s, addr[0], addr[1], addr[2], addr[3] , port, len);
	debug32("Fail[invalid ip,port]\r\n");
#endif
	}
	else
...
uint16 recvfrom(SOCKET s, uint8 * buf, uint16 len, uint8 * addr, uint16 *port)
{
	...
	if ( len > 0 )
	{
...
#ifdef __DEF_IINCHIP_DBG__
   	debug32("ISR_RX: rd_ptr : %.4x\r\n", ptr);
#endif
   	switch (IINCHIP_READ(Sn_MR(s)) & 0x07)
   	{
   	case Sn_MR_UDP :
   			read_data(s, (uint8 *)(uint32)ptr, head, 0x08);
   			ptr += 8;
...
#ifdef __DEF_IINCHIP_DBG__
   			debug32("UDP msg arrived\r\n");
   			debug32("source Port : %d\r\n", *port);
   			debug32("source IP : %d.%d.%d.%d\r\n", addr[0], addr[1], addr[2], addr[3]);
#endif

			read_data(s, (uint8 *)(uint32)ptr, buf, data_len); // data copy.
			ptr += data_len;
   	...
   	case Sn_MR_IPRAW :
   			read_data(s, (uint8 *)(uint32)ptr, head, 0x06);
   			ptr += 6;
			...
			IINCHIP_WRITE((Sn_RX_RD0(s) + 1),(uint8)(ptr & 0x00ff));
   			break;
   	case Sn_MR_MACRAW :
   			read_data(s,(uint8*)(uint32)ptr,head,2);
   			ptr+=2;
   			data_len = head[0];
   			data_len = (data_len<<8) + head[1] - 2;
   			if(data_len > 1514) 
   			{
   				debug32("data_len over 1514\r\n");
   				while(1);
   			}

   			read_data(s,(uint8*)(uint32) ptr,buf,data_len);
   			ptr += data_len;
   			IINCHIP_WRITE(Sn_RX_RD0(s),(uint8)((ptr & 0xff00) >> 8));
   			IINCHIP_WRITE((Sn_RX_RD0(s) + 1),(uint8)(ptr & 0x00ff));
   			
#ifdef __DEF_IINCHIP_DGB__
			debug32("MAC RAW msg arrived\r\n");
			debug32("dest mac=%.2X.%.2X.%.2X.%.2X.%.2X.%.2X\r\n",buf[0],buf[1],buf[2],buf[3],buf[4],buf[5]);
			debug32("src  mac=%.2X.%.2X.%.2X.%.2X.%.2X.%.2X\r\n",buf[6],buf[7],buf[8],buf[9],buf[10],buf[11]);
			debug32("type    =%.2X%.2X\r\n",buf[12],buf[13]); 
#endif			
			break;
